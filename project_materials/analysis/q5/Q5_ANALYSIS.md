...
### 7.1 与 Q4 最终预测的直接数值比较

Q4 和 Q5 预测的是完全相同的 2015-01-04 至 2015-01-10 时段，因此还可以比较两组正式预测本身的水平变化：

| Area | 指标 | Q4 无天气 | Q5 含天气 | Q5 - Q4 | 相对变化 |
|---|---|---:|---:|---:|---:|
| Area 1 | 7日平均负荷 | 6536.03 MW | 6478.70 MW | -57.33 MW | -0.88% |
| Area 1 | 7日最高预测值 | 8535.24 MW | 8613.30 MW | +78.06 MW | +0.91% |
| Area 1 | 7日最低预测值 | 4101.94 MW | 3932.17 MW | -169.77 MW | -4.14% |
| Area 2 | 7日平均负荷 | 6834.78 MW | 6709.67 MW | -125.11 MW | -1.83% |
| Area 2 | 7日最高预测值 | 8524.40 MW | 9022.10 MW | +497.70 MW | +5.84% |
| Area 2 | 7日最低预测值 | 4031.23 MW | 4018.07 MW | -13.16 MW | -0.33% |

这表明天气信息并不是简单地把整条 Q4 曲线整体上移或下移，而是在不同日期和时段对负荷轮廓进行重新调整。Area 2 的预测峰值变化尤其明显，与 Q3 中 Area 2 对温度更敏感、Q5 中天气增益更大的结果一致。

但这些 Q4/Q5 预测值之间的差异**不能直接当作精度改善证据**，因为目标周真实负荷未知。精度改善的主要证据仍应来自相同训练窗口、相同 rolling-origin 验证周下的受控 A/B 回测：Area 1 MAPE 从 7.00% 降至 5.71%，Area 2 从 7.62% 降至 5.98%。
...
- **Area 1**：加入天气后，受控回测平均 MAPE 从 7.00% 降至 5.71%，相对改善约 18.35%；
- **Area 2**：加入天气后，受控回测平均 MAPE 从 7.62% 降至 5.98%，相对改善约 21.55%；
- 两个地区都从天气信息中获得了明确增益，且 Area 2 的相对提升更大；
- `mean_temp_quad_hr` 在两个地区都优于单纯平均温度模型和全量天气模型，因此最终采用“平均温度 + 平均温度² + 湿度 + 降雨”的规格，而不是机械加入所有气象变量。

因此，本题可以回答为：**计及气象因素后，两个地区的历史外推预测精度均得到改善；Area 2 的改善幅度更大。**
...
题目附录要求最终提交 `Q5_Area1_Load.xlsx` 和 `Q5_Area2_Load.xlsx`。当前仓库已保存完整的 7×96 预测 CSV 作为可审计中间结果；XLSX 是提交格式转换问题，不影响本节模型选择、回测结论与预测数值。
